<?php
// =============================================================================
// ChainTrack — Location Management: Edit Location
// locations/edit.php
// =============================================================================
...
$v = [
    ...
    'department_id' => $loc['department_id'] ? (string)$loc['department_id'] : '',
    'description'   => $loc['description'] ?? '',
    'is_active'     => (int)$loc['is_active'],
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($v['location_name'] !== '' && !isLocationNameUnique($db, $v['location_name'], $id)) {
        $errors[] = "A location named \"{$v['location_name']}\" already exists.";
    }

    if ($v['location_code'] !== '' && !isLocationCodeUnique($db, $v['location_code'], $id)) {
        $errors[] = "The location code \"{$v['location_code']}\" is already assigned.";
    }

    if ($v['parent_id'] !== '' && (int)$v['parent_id'] === $id) {
        $errors[] = "A location cannot be its own parent facility.";
    }
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $db->prepare(
                "UPDATE locations
                 SET location_code = ?, location_name = ?, location_type = ?,
                     parent_id = ?, department_id = ?, description = ?, is_active = ?
                 WHERE id = ?"
            );
            $stmt->bind_param('sssissii', $code, $v['location_name'], $v['location_type'], $parent, $dept, $desc, $active, $id);
            $stmt->execute();
            $stmt->close();

            logAudit($db, sessionUserId(), 'location.updated', 'location', $id, "Updated location {$v['location_name']} (ID: {$id})");

            $_SESSION['flash'] = [
                'type'    => 'success',
                'message' => "Location \"{$v['location_name']}\" updated successfully.",
            ];
            header('Location: /locations/index.php');
            exit;
        } catch (Throwable $e) {
...
// Dropdown options (exclude self from parent list)
$allLocations = $db->query("SELECT id, location_name, location_code FROM locations WHERE is_active = 1 AND id != {$id} ORDER BY location_name ASC")->fetch_all(MYSQLI_ASSOC);
...
$pageTitle = 'Edit Location: ' . $loc['location_name'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <meta name="description" content="Edit storage facility or examination location.">
...
<div id="ct-main">
  ...
  <div id="ct-topbar">
    <span class="page-title">Edit Location</span>
  ...
  <div id="ct-content">
    ...
    <div class="mb-2">
      <div class="small text-muted mb-1">
        <a href="/locations/index.php" style="color:var(--ct-muted);text-decoration:none;">Locations</a> › <span>Edit</span>
      </div>
      <h2 style="margin:0;font-size:1.25rem;">Edit Location: <?= esc($loc['location_name']) ?></h2>
    </div>

    <div class="ct-card" style="max-width:720px;">
      <div class="ct-card-header">Location Details (ID: #<?= (int)$id ?>)</div>
      <div class="ct-card-body">
        ...
        <form method="POST">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
            <div class="ct-form-group">
              <label class="ct-label">Parent Facility</label>
            ...
            <div class="ct-form-group">
              <label class="ct-label">Assigned Department</label>
          ...
          <div class="ct-form-group">
            ...
            <textarea name="description" class="ct-textarea" style="min-height:80px;"><?= esc($v['description']) ?></textarea>
          </div>

          <div class="d-flex gap-2 mt-2">
            <button type="submit" class="ct-btn ct-btn-primary">Update Location</button>
